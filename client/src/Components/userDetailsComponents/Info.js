...
const Info = () => {
  ...
  return (
    <>
      {fromTo && seatData ?
        <div className="flex sm:flex-row flex-col p-7 gap-5">
          <div>
            <div className="flex sm:h-44 h-30 sm:mt-4 sm:ml-16 rounded-[10px] border-2 border-solid border-[rgb(169,169,169)] w-[85vw] sm:w-[900px]">
              <div className="flex flex-col justify-between sm:w-[950px] w-[300px] p-2 sm:p-5 ">
                <div className="flex flex-col sm:gap-2.5">
                  ...
                  <div className='flex sm:gap-4 gap-1'>
                    <h1 className='sm:text-lg text-[12px] font-semibold self-center text-slate-700'>{seatData.selectedBusCard.startTime}, {formattedDate}</h1>---
                    <h1 className='sm:text-sm text-[8px] font-semibold self-center text-slate-500'>07 hrs 58 min</h1>---
                    <h1 className='sm:text-lg text-[12px] font-semibold self-center text-slate-700'>{seatData.selectedBusCard.EndTime}, {formattedDate}</h1>
                  </div>
                  <div className="flex sm:gap-52 gap-10">
                    <p className="sm:text-lg text-[12px] font-semibold">From: {fromTo.From}</p>
                    <p className="sm:text-lg text-[12px] font-semibold">To: {fromTo.To}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-4 h-full w-[320px] mt-4 p-5 mr-10 rounded-[10px] border-2 border-solid border-[rgb(169,169,169)]">
            ...
            <button onClick={() => handleFinalSubmit()} className="bg-[rgba(233,115,72,0.961)] h-[45px] text-[white] rounded-[5px]">
              Proceed To Payment
            </button>
          </div>
        </div>
