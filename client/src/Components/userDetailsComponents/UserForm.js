...
export default function UserForm(props) {
  ...
  return (
    ...
    >
      {(formik) => (
        <>
        <div className="sm:ml-16 mt-4 sm:w-[900px] w-[85vw]">
              <h2 className="font-semibold pl-2 sm:text-[x-large] text-[15px]">
                Enter Traveller Details
              </h2>
          <Form>
            {formik.values.users.map((user, index) => (
              <section key={index} className='bg-white mr-1 m-6 flex flex-col gap-1 border-2 border-slate-400 rounded-lg p-4'>
                <h1 className='text-lg font-semibold '>Passanger Details for seat {users[index]}</h1>
              <div className="flex sm:flex-row flex-col gap-2 sm:gap-[30px] mt-4">
                  <div className='flex flex-row justify-between sm:flex-col gap-2.5'>
                      ...
                      className='sm:w-[400px] w-[50vw]  px-3 h-10 rounded-[5px] border-2 border-solid border-[rgb(169,169,169)]'
                      />
                      <ErrorMessage  name={`users[${index}].name`} />
                  </div>
                  <div className="flex justify-between sm:flex-col flex-row  gap-2.5">
                      <label className='text-[rgb(112,105,105)] self-center  sm:self-start' htmlFor={`users[${index}].gender`}>Gender</label>
                      <Field disabled={isFormDisabled} className="sm:w-[150px] w-[50vw] h-10 pl-[5px] rounded-[5px] border-2 border-solid border-[rgb(169,169,169)]" name={`users[${index}].gender`} as="select">
                  ...
                  <div className="flex justify-between sm:flex-col flex-row gap-2.5">
                      ...
                      <Field disabled={isFormDisabled} type="number" name={`users[${index}].age`}
                       className="sm:w-[150px] w-[50vw] px-3  h-10 rounded-[5px] border-2 border-solid border-[rgb(169,169,169)]"
                      />
                      <ErrorMessage  name={`users[${index}].age`} />
                  </div>
              </div>
                <div className="flex sm:flex-row flex-col gap-[30px] mt-4">
                  <div  className="flex justify-between sm:flex-col flex-row gap-2.5">
                      ...
                      className="px-3 sm:w-[400px] w-[50vw] h-10 rounded-[5px] border-2 border-solid border-[rgb(169,169,169)]"
                      />
                      <ErrorMessage name={`users[${index}].email`} />
                  </div>
                  <div className="flex justify-between sm:flex-col flex-row gap-2.5">
                      <label htmlFor={`users[${index}].phone`} className="text-[rgb(112,105,105)] self-center  sm:self-start">Phone</label>
                      <Field disabled={isFormDisabled}
                      type="number"
            ...
            {isFormDisabled?
            <button  type='submit'
            className='m-6 border-2 bg-slate-400 text-white font-semibold font-mono px-2 py-1 rounded-lg my-4 text-xl'>
            Submit
          </button>
            :
            <button type='submit'
              className='m-6 border-2 bg-[#FF8700] text-white font-semibold font-mono px-2 py-1 rounded-lg my-4 text-xl'>
              Submit
            </button>
            }
